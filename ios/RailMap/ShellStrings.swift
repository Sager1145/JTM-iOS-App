...
///
/// Shared concepts continue to come from the main catalog (`Localizable.json`,
/// which both products read); this table is intentionally limited to labels
/// the iPhone app invented — tab titles the system `TabView` has no room for
/// the catalog's full wording of, and the chrome around them.
///
/// One of ``AppStrings``' five contributors, and the only one that is not
/// named after a screen: these are the shell's own words, used from more than
/// one workspace. Adding a key here rather than to a screen's table is the
/// right call exactly when two screens would otherwise both need it.
enum ShellStrings {

...
- Toronto (2026-09-04): the City subway route layer and the TTC GTFS shape are one schematic geometry (up to 135 m off track); Lines 1 and 2 are drawn from OSM relations 102388/102386 accepted through `osmRelationEvidenceByRouteId`, validated against the City's topographic subway-track survey by `app/scripts/railway/validate-ttc-subway-osm.py`. The streetcar groups are named after the 5xx day routes (`primaryRouteIds`), and `excludeTripHeadsignPattern` keeps the TTC's replacement-bus trips out of pattern selection. Re-run the validator before touching either subway line.

...
// Identity of the record bundle / marker record array LAST UPLOADED to the
// GPU. A cache hit above returns the same object, so an unchanged render can
// skip the upload entirely — which only holds while these two are cleared in
// the same breath as the caches they guard. That is why they live here with
// the invalidators rather than in app-route-render.js, which merely reads and
// re-stamps them after a push.
...
// --- corridor-fit primitives ------------------------------------------------
// The B-spline corridor solve below is a pipeline of named intermediate
// products. Each primitive here builds exactly one of them and keeps no
// state, so smoothCorridorCurveUncached reads as the list of stages it is.
...
// lon/lat ⇄ local metric offsets from `origin`; mx/my are the metres-per-
// degree scales at the corridor's mean latitude.
function metricOffsetsFrom(points, origin, mx, my) {
  return points.map((p) => [(p[0] - origin[0]) * mx, (p[1] - origin[1]) * my]);
}

function pointsFromMetricOffsets(metric, origin, mx, my) {
  return metric.map((p) => [origin[0] + p[0] / mx, origin[1] + p[1] / my]);
}

// INTERMEDIATE PRODUCT: `anchors` — the source corridor resampled to `count`
// evenly spaced points. Every later stage is deviation-clamped against these,
// so the fit can never wander away from the physical track.
//
...
function resampleCorridorAnchors(line, cum, total, count) {
  ...
  for (let i = 0; i < count; i += 1)
    anchors[i] = pointOnSource((total * i) / (count - 1));
  return anchors;
}

// The polyline counterpart of gaussianSmoothSeries.
//
...
function gaussianSmoothPolyline(input, sigma, step) {
  const radius = Math.max(
    3,
    Math.min(240, input.length - 1, Math.ceil((sigma * 3) / step)),
  );
  const weights = new Array(radius + 1);
  for (let k = 0; k <= radius; k += 1)
    weights[k] = Math.exp(-0.5 * Math.pow((k * step) / sigma, 2));
...
// Pull `p` back onto the maxDeviation disc centred on its source anchor.
function clampMetricDeviation(p, anchor, maxDeviation) {
...
// INTERMEDIATE PRODUCT: the radius-relaxed metric polyline.
//
...
function relaxMetricToMinRadius(
  metric,
  anchorMetric,
  minRadius,
  maxDeviation,
  ...
  for (let pass = 0; pass < 48; pass += 1) {
    ...
    for (let i = curvatureHalf; i < metric.length - curvatureHalf; i += 1) {
      ...
      const candidate = clampMetricDeviation(
...
// INTERMEDIATE PRODUCT: `controls` — the B-spline control polygon.
//
...
function buildSplineControlPolygon(metric, anchorMetric, opts) {
  const { knotEvery, total, minDetail, minRadius } = opts;
  let controls = [];
...
// INTERMEDIATE PRODUCT: `count` evenly parameterised samples of the clamped
// cubic B-spline through `controls`.
//
...
function sampleClampedBSpline(controls, count) {
  const degree = 3;
  ...
  for (let i = 0; i < count; i += 1) out[i] = splinePoint(i / (count - 1));
  return out;
}

// INTERMEDIATE PRODUCT: the smallest circumradius measured along `points`,
// over a window scaled to the requested minimum detail.
function measureMinCircumRadius(points, minDetail) {
  let length = 0;
  for (let i = 1; i < points.length; i += 1)
    length += Math.hypot(
...
// INTERMEDIATE PRODUCT: a chord-pulled solution ({ spline, radius }).
//
...
function pullControlsTowardChord(controls, anchorMetric, spline, radius, opts) {
  const { minRadius, maxDeviation, minDetail, solveN } = opts;
  const originalControls = controls.map((p) => p.slice());
  let bestSpline = spline;
  let bestRadius = radius;
  for (let pass = 1; pass <= 32; pass += 1) {
    const f = pass / 32;
    const passControls = originalControls.map((p, i) => {
      ...
      const target = clampMetricDeviation(
        chord,
        anchorMetric[ai],
        maxDeviation,
      );
    ...
    const candidate = sampleClampedBSpline(passControls, solveN);
    const candidateRadius = measureMinCircumRadius(candidate, minDetail);
  ...
  return { spline: bestSpline, radius: bestRadius };
}

// INTERMEDIATE PRODUCT: `bestArc` — the best constant-curvature circular arc
// candidate, or null when none satisfies the deviation budget.
//
...
function fitCircularArcFallback(splineMetric, anchorMetric, opts) {
  const { total, minRadius, maxDeviation, solveN } = opts;
...
// INTERMEDIATE PRODUCT: `angles` — the radius-limited direction field, a
// continuous scalar angle per output sample.
function directionFieldAngles(cur, coslat, opts) {
  const { minDetail, minRadius, outputStep, directionSigma } = opts;
  ...
  angles = gaussianSmoothSeries(angles, directionSigma, outputStep, 3);
  angles = gaussianSmoothSeries(angles, directionSigma * 0.65, outputStep, 3);
...
// INTERMEDIATE PRODUCT: `integrated` — geometry rebuilt from the direction
// field, with the endpoint drift distributed linearly over the whole arc.
//
...
function integrateFromDirectionField(splineMetric, angles) {
...
function minDirectionRadius(angles, cum) {
  let radius = Infinity;
  for (let i = 1; i < angles.length; i += 1) {
    const turn = Math.abs(angles[i] - angles[i - 1]);
    if (turn > 1e-9)
      radius = Math.min(radius, (cum[i] - cum[i - 1]) / turn);
  }
  return radius;
}

// INTERMEDIATE PRODUCT: the display-resolution resampling of an accepted
// curve — points, interpolated angles and the matching arc-length table.
function resampleFittedCurveOutput(
  cur,
  angles,
  smoothCum,
  smoothTotal,
  ...
  const outputPoints = new Array(displayN);
  const outputAngles = new Array(displayN);
  let sourceIndex = 0;
  ...
  return {
    points: outputPoints,
    angles: outputAngles,
    cum: cumulativeMeters(outputPoints),
  };
}
...
function smoothCorridorCurveUncached(line) {
  ...
  const anchors = resampleCorridorAnchors(line, cum, total, workN);
  ...
  const anchorMetric = metricOffsetsFrom(anchors, origin, mx, my);
  ...
  metric = gaussianSmoothPolyline(metric, sigmaM, workStep);
  metric = gaussianSmoothPolyline(metric, sigmaM * 0.65, workStep);
  metric = metric.map((p, i) =>
    clampMetricDeviation(p, anchorMetric[i], maxDeviation),
  );
  ...
  const controls = buildSplineControlPolygon(metric, anchorMetric, {
    knotEvery,
    total,
    minDetail,
    minRadius,
  ...
  let splineMetric = sampleClampedBSpline(controls, solveN);
  let achievedMinRadius = measureMinCircumRadius(splineMetric, minDetail);

  // ── §6 fallback 1: pull the whole control polygon toward its chord ──
  if (achievedMinRadius < minRadius * 0.999) {
    const pulled = pullControlsTowardChord(
      controls,
      anchorMetric,
      splineMetric,
      achievedMinRadius,
      { minRadius, maxDeviation, minDetail, solveN },
    );
  ...
  if (achievedMinRadius < minRadius * 0.999) {
    const bestArc = fitCircularArcFallback(splineMetric, anchorMetric, {
      total,
      minRadius,
      maxDeviation,
      solveN,
    ...
    if (bestArc) {
      ...
      achievedMinRadius = measureMinCircumRadius(splineMetric, minDetail);
      usedCircularArc = true;
    }
  }

  // ── §8 back to lon/lat: arc length and step of the accepted geometry ──
  let cur = pointsFromMetricOffsets(splineMetric, origin, mx, my);
  let smoothCum = cumulativeMeters(cur);
  ...
  const angles = directionFieldAngles(cur, coslat, {
    minDetail,
    minRadius,
    outputStep,
    directionSigma: directionSigmaM,
  });
  const integrated = integrateFromDirectionField(splineMetric, angles);
  const integratedRadius = measureMinCircumRadius(integrated, minDetail);
  if (integratedRadius >= achievedMinRadius) {
    splineMetric = integrated;
    achievedMinRadius = integratedRadius;
    cur = pointsFromMetricOffsets(splineMetric, origin, mx, my);
    smoothCum = cumulativeMeters(cur);
    smoothTotal = smoothCum[smoothCum.length - 1];
    outputStep = smoothTotal / Math.max(1, cur.length - 1);
  }
  const achievedDirectionRadius = minDirectionRadius(angles, smoothCum);
  ...
  if (displayN !== cur.length) {
    const resampled = resampleFittedCurveOutput(
      cur,
      angles,
      smoothCum,
      smoothTotal,
  ...
  const outputMetric = metricOffsetsFrom(outputPoints, origin, mx, my);
  let finalAchievedMinRadius = measureMinCircumRadius(outputMetric, minDetail);
  let finalAchievedDirectionRadius = minDirectionRadius(
    outputAngles,
    outputCum,
  );
  if (
...
// --- station-join primitives ------------------------------------------------
// smoothCurveStationJoins is a five-stage pipeline over fitted curves; each
// primitive below builds exactly one of its intermediate products.

// INTERMEDIATE PRODUCT: `owners` — fitted curve → the groups that reference
// it. A station-continuous curve is shared, so the map is many-to-one.
...
// INTERMEDIATE PRODUCT: `ends` — one descriptor per curve endpoint carrying
// its outward tangent, its member train ids and its snapped node key.
function collectStationJoinEndpoints(curves, owners) {
  const ends = [];
...
// INTERMEDIATE PRODUCT: `candidates` — plausible endpoint pairs, best first.
function pairStationJoinCandidates(ends) {
  const candidates = [];
...
// INTERMEDIATE PRODUCT: `connections` — curve → side → the accepted partner.
// Greedy one-to-one selection keeps a fork from claiming both sides.
function buildStationJoinConnections(candidates) {
...
// INTERMEDIATE PRODUCT: `components` — station-continuous chains, each an
// ordered [{ curve, reverse }] list starting from a degree-1 end. A cycle
// needs a periodic solver, so it is marked visited and never emitted.
function walkStationJoinChains(curves, connections) {
...
// INTERMEDIATE PRODUCT: one chain's concatenated `source` polyline plus the
// join diagnostics (`joinEdges`, worst turn and worst gap) the failure
// report and the accepted curve's stamps are built from.
function concatStationJoinComponentSource(component, connections) {
  const source = [];
...
// INTERMEDIATE PRODUCT: the accept/reject verdict on a fitted chain.
//
...
function stationJoinConstraintReport(fitted) {
  const radiusOk = !(
...
// Five stages: owners → endpoints → candidate pairs → connections → chains,
// then one refit per chain.
function smoothCurveStationJoins(groupInfo) {
  if (!groupInfo || groupInfo.size < 2)
    return { roundedJoins: 0, failures: [] };
  const owners = indexStationJoinCurveOwners(groupInfo);
  const curves = [...owners.keys()];
  const ends = collectStationJoinEndpoints(curves, owners);
  const candidates = pairStationJoinCandidates(ends);
  const connections = buildStationJoinConnections(candidates);
  const components = walkStationJoinChains(curves, connections);

  let roundedJoins = 0;
  const failures = [];
  components.forEach((component) => {
    const { source, joinEdges, worstOriginalTurn, worstGap } =
      concatStationJoinComponentSource(component, connections);
    ...
    const { radiusOk, deviationOk, directionOk, radiusRelaxed } =
      stationJoinConstraintReport(fitted);
